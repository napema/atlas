...
function aggiornaStep(container, step, totale) {
  ...
  // Il lato entra nel titolo: leggerlo solo in una mo-chip non bastava.
  ...
  const chip = [];
  if (d.fonte) chip.push(`<span class="mo-chip is-verde">${d.fonte}</span>`);
  const etichettaTag = { M: "Attivo", S: "Passivo", R: "Rilascio" }[d.tag] || "";
  const classeTag = { M: "is-verde", S: "is-blu", R: "is-arancio" }[d.tag] || "";
  if (etichettaTag) chip.push(`<span class="mo-chip ${classeTag}">${etichettaTag}</span>`);
  if (d.badgeExtra) chip.push(`<span class="mo-chip is-arancio">${d.badgeExtra}</span>`);
  if (d.lato) chip.push(`<span class="mo-chip is-blu">Lato ${nomeLato(d.lato).toLowerCase()}${d.extra ? " · extra" : ""}</span>`);
  if (d.carico) chip.push(`<span class="mo-chip is-arancio">${d.carico} kg</span>`);
  if (d.ripetizioni) chip.push(`<span class="mo-chip is-arancio">${d.ripetizioni}</span>`);
  if (d.serve) chip.push(`<span class="mo-chip">${d.serve}</span>`);
  for (const m of d.muscoli || []) chip.push(`<span class="mo-chip">${m}</span>`);
