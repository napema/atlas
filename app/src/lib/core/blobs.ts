// blobs.ts — archivio dei file binari (foto dei progressi, ricevute, allegati).
//
...
function transazione(modo: IDBTransactionMode) {
  return apri().then((db) => db.transaction(DEPOSITO, modo).objectStore(DEPOSITO));
}

function attendi<T>(req: IDBRequest<T>): Promise<T> {
 ...
 */
export async function salva(id: string, modulo: string, blob: Blob, extra: Record<string, unknown> = {}) {
...
export async function leggi(id: string): Promise<Blob | null> {
  const deposito = await transazione("readonly");
  const r = await attendi<{ blob: Blob } | undefined>(deposito.get(id));
  return r ? r.blob : null;
}

/** URL temporaneo per un tag <img>. Va revocato con `liberaURL` quando basta. */
export async function urlDi(id: string) {
  const b = await leggi(id);
  return b ? URL.createObjectURL(b) : null;
}

export const liberaURL = (url: string | null | undefined) => { if (url) URL.revokeObjectURL(url); };
...
export async function idDelModulo(modulo: string) {
  const deposito = await transazione("readonly");
